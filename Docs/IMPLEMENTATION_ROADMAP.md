...
状态（2026-07-24）：任务035–043的非GPU实现与自动化检查已完成；任务038–043的目标浏览器性能/视觉验收，以及任务024–032的GPU训练和正式实验仍待执行。

- 已接入OpenStreetMap街道图与免密钥卫星图，并将SUMO局部坐标投影到地图参考点。
- 已实现最多100辆车、5个事件的编辑、渲染和JSON交换；当前AI模型运行边界仍为50辆车、2个事件。
- 优化推理、状态传输和前端渲染；必要时降低推送频率或使用Canvas。
- 已实现Three.js 3D鸟瞰视图和2D/3D同步切换；NS-3仍作为可选后续能力。
- 已将WebSocket降为关键帧数据源，由独立动画引擎持续插值车辆、粒子和慢动作；车辆、消息、危险效果与智能镜头在2D/3D中共享状态语义。
- 已实现基于真实仿真数据的八阶段60秒叙事，支持暂停、手动推进、AI/基线先叠加后分屏；叙事墙钟不依赖后端仿真速度。

验收：在目标Mac Chrome 1920×1080记录50辆车、10条并发消息和组合效果的帧率，并走完60秒叙事；同时记录车辆规模、真实推理时间、内存和训练时长。若未达到目标，降低视觉粒子密度或保留50辆车场景作为正式演示配置。
