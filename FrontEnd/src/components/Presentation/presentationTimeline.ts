...
export function comparisonMetrics(pair: ComparisonPair | null, networkMetricsAvailable = true): ComparisonMetric[] {
  if (!pair) {
    return [
      ...
      { label: '通信负载降低', baseline: '未计算', ai: '未计算' },
      { label: '估算时延变化', baseline: '未计算', ai: '未计算' },
      { label: '消息送达率', baseline: '未计算', ai: '未计算' },
    ]
  }
  ...
  return [
    ...
    {
      label: '消息送达率',
      baseline: networkMetricsAvailable ? metricValue(pair.baseline.metrics.delivery_rate * 100, '%') : '未计算',
      ai: networkMetricsAvailable ? metricValue(pair.ai.metrics.delivery_rate * 100, '%') : '未计算',
      delta: networkMetricsAvailable
        ? percentChange(pair.baseline.metrics.delivery_rate, pair.ai.metrics.delivery_rate) : undefined,
    },
  ]
}
...
export function stageMetrics(
  ...
  notifiedVehicles: number,
): Array<{ label: string; value: string }> {
  return [
    { label: '通知车辆', value: String(notifiedVehicles) },
    { label: '平均时延', value: metrics ? `${metrics.avg_delay_ms.toFixed(1)} ms` : '未计算' },
    { label: '送达率', value: metrics ? `${(metrics.delivery_rate * 100).toFixed(1)}%` : '未计算' },
  ]
}
